...
const Masonry = ({
    ...
    // Calculate total height of the grid to set the container height
    const containerHeight = Math.max(...(grid.length > 0 ? grid.map(i => i.y + i.h) : [0]));
    ...
    return (
        <div ref={containerRef} className="list" style={{ minHeight: containerHeight > 0 ? containerHeight : '50vh' }}>
            {grid.map(item => {
                return (
                    <div
                        key={item.id}
